...
data_white_train <- data_white_train %>% 
...
data_black_train <- data_black_train %>% 
  mutate(selectROC=c("omitted", "control", "case")[1+(ObesityClass=="Normal weight")+2*(ObesityClass=="Obese")]) %>%
...
plotSelectionROC <- ggplot(bind_rows(data_white_train, data_black_train), 
                           aes(x=predicted, y=exp(BMI))) + 
  geom_point(aes(col=selectROC, pch=Race)) +
  scale_color_manual(values=c("#FF0000", "#00CCCC", "lightgray")) +
  labs(title="Selection for ROC analysis (training data)", 
       subtitle=paste0("strat. pred., effects: ", formulationBlack$effect, 
                       ", type: ", formulationBlack$type,
                       ", BMI transformation: ", formulationBlack$transformation),
...
# reclassification in the test set
...
plotSelectionReclassify <- ggplot(bind_rows(data_white_test, data_black_test), 
  ...
  labs(title="Test set reclassification",
       subtitle=paste0("strat. pred., effects: ", formulationBlack$effect, 
                       ", type: ", formulationBlack$type,
                       ", BMI transformation: ", formulationBlack$transformation),
       x="predicted BMI", y="observed BMI")
...
convertToTexTable(ROCstats, "reclassify_ROCstats.tex", rows.named=TRUE, minipage=TRUE,
                  caption="ROC performance of metabolic BMI predicting observed BMI classes of normal weight versus obese, AUC + 95\\% CI.", 
                  reflabel="reclassify_ROCstats")

trueRatesAll <- data.frame(stratum="All", TPR=rocTrainAll$sensitivities, 
                           TNR=1-rocTrainAll$specificities, threshold=rocTrainAll$thresholds)
trueRatesWhite <- data.frame(stratum="White", TPR=rocTrainWhite$sensitivities, 
                             TNR=1-rocTrainWhite$specificities, threshold=rocTrainWhite$thresholds)
trueRatesBlack <- data.frame(stratum="Black", TPR=rocTrainBlack$sensitivities, 
                             TNR=1-rocTrainBlack$specificities, threshold=rocTrainBlack$thresholds)
trueRates <- bind_rows(trueRatesAll, trueRatesWhite, trueRatesBlack)

thresholdStats <- bind_rows(subset(trueRatesAll, subset = threshold==cutoffAll),
                            subset(trueRatesWhite, subset = threshold==cutoffWhite),
                            subset(trueRatesBlack, subset = threshold==cutoffBlack)
                            )

ROCcurves <- ggplot(trueRates, aes(x=TNR, y=TPR, by=stratum)) + 
  geom_path(aes(color=stratum)) +
  geom_point(aes(color=stratum), data=thresholdStats) +
  scale_color_manual(values=c("black", "blue", "red")) +
  geom_segment(aes(x=0, y=0, xend=1, yend=1), lty="dashed") +
  labs(title="Detection rates observed normal weight versus obese",
       subtitle="training set data")
...
data_white <- merge(data_white, predict_white, by="ID", sort=FALSE)
...
data_black <- merge(data_black, predict_black, by="ID", sort=FALSE)
